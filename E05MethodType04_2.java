...
/*
시나리오] 여러개의 숫자를 입력받은 후 최댓값을 구하는 메소드를 정의하시오.
	단, 숫자의 갯수는 매개 변수로 전달받고 숫자의 갯수만큼 Scanner클래스를 통해 입력받는다.
...
public class E05MethodType04_2 {
	...
	public static void main(String[] args) {
		...
		System.out.println("최댓값1:"+ maxValue2);
	}
	...
	static int returnMaxNumber(int numberCnt) {
		...
		for(int i =1; i<=numberCnt; i++) {
			...
			if(i==1) {
				//입력받은 숫자중 첫번째 값을 '기준값'(=>비교의 대상_값)으로 설정한다.
				maxVal = inputNum;
			}
			else {
				...
				//maxVal이 더 크다면 기존값을 유지하면 되므로 else는 필요 X
			}
		}
