...
public class E01JunsuType {
	...
	public static void main(String[] args) {
		...
		-일반적으로 CPU(중앙처리장치)는 정수형 연산을 할 때
			int형을 가장 빠르게 처리한다.
		-즉, 정수형 변수를 쓸 때는 거의 대부분 int를 사용한다.
		 */
		int int1 = 100;
		int int2 = 200;
		...
		/ 연산은 두 정수를 나눠서 '몫'을 구하는 연산자이므로 결과는 0이 나온다.
		 */
		result1 = int1/int2;
		System.out.println("int1/int2="+result1);
		...
		-byte, short형은 게임 캐릭터의 움직임의 표현이나 음원정보 등을 저장할 때 주로 사용한다.
		 */
		...
		int result3=short1+short2; //short + short = int형 // short와 byte는 int보다 작다.
		System.out.println("short1+short2="+result3);
		/*
		-동일자료형끼리의 연산은 동일자료형이 되는 것이 기본 원칙이다.
		-큰 자료형과 작은 자료형을 연산하면 큰 자료형으로 자동형 변환된다.
		(데이터의 손실이 있을 수 있기 때문)
		-단, byte와 short는 예외적인 상황으로 생각하면 된다. (int형에 최적화된 cpu의 특성 때문임)
		 */
		long long1=1000, long2=2000;
		long result4=int1+long1; //int+long=long
		System.out.println("int1+long1="+result4);
		
		result4 = long1+long2; //long끼리의 연산이므로 기본규칙 따른다.
		System.out.println("long1+long2="+result4);
		
		/*
		자바 컴파일러는 정수형상수를 기본적으로 int형으로 간주하기 때문에
		위 정수를 메모리에 올리는 순간 error가 발생한다.
		이때에는 접미사(L or l)을 붙여서 int형 자료가 아닌 long형 자료임을 알려줘야한다.
