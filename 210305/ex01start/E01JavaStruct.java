...
//자바파일의 구조
/*
 1. 패키지 선언부 : 클래스의 종류별로 묶어서 관리하기 위한 선언으로
 	디렉토리(폴터)의 의미로 이해해도 된다.
 	2.improt 선언부 : 내가 만든 클래스에 필요한 자바클래스를 가져다
 	쓰기 위한 선언으로 JDK에서 재공하는 클래스를 사용할 수도 있다.
 	3.클래스 선언부 : 자바 프로그램은 클래스 단위로 구성되므로 기본적으로
 	필요한 부분으로 클래스는 항상 영문 대만자로 시작하고 파일의
 	확장자는 .java로 해야한다.
 	*/
