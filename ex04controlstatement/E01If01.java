...
if문(조건문, 분기문)
	형식1
		...
		-if문의 조건문은 반드시 비교식 혹은 논리식이어야 한다. (산술식 X)
		-즉, 결과값이 boolean타입(true, false)이어야 한다.
		-실행할 문장이 '하나'인 경우 중괄호 생략이 가능.
*/
...
//		}
		...
		/*
		if(조건식); => 조건식에 상관없이 다음 문장이 실행되는 형태로 세미콜론은 문장의 끝을 나타내므로
			사용에 주의해야 한다. 즉, if문에는 ; 쓰지마라.
		 */
		if(num%2 != 0);
		{
		...
		단, 실행문이 2개 이상인 경우에는 한 문장만 실행되므로 반드시 중괄호를 작성.
		 */
		if(num%2 == 0)
			System.out.println("\n"+num+"은 짝수이다.");
		if(num%2 != 0) {
		...
		/*
		System.in.read()
			: 사용자에게 하나의 문자를 입력받을 때 사용하는 메소드로 입력한 문자의 아스키 코드값을 반환한다.
			해당 함수는 IOException을 기본적으로 가지고 있어 이를 무시하기 위해 main()메소드에서
			throws 처리를 해준다.
		 */
...
//		System.out.println("입력한문자:"+(char)asciiCode);
		...
		/*
		시나리오] 사용자가 입력한 문자가 숫자인지 판단하는 프로그램을 if문을 이용해서 구현하시오.
		 */
		
		/*
		아스키코드값을 알고 있을 때의 구현방법
		'0' => 48
		'9' => 57 이므로 해당 구간 안에 있다면 숫자로 판단할 수 있다.
		
		※숫자의 구간을 정할 때는 '반드시' 논리 And를 사용해야 한다.
		 */
		System.out.print("하나의 문자를 입력하세요:");
		int asciiCode = System.in.read();
