...
/*
do~while문
	: 반드시 한 번은 실행해야할 경우 사용하는 반복문.
	즉, 조건 검사없이 무조건 한 번은 실행된다.
	형식]
		반복을 위한 변수;
		do{
			실행문장;
			반복문을 탈출하기 위한 증감식;
		} while(조건식); <= 반드시 문장 끝에는 세미콜론이 있어야한다. //앞에서 배운 while과의 차이점
 */
public class E04DoWhile {
	...
	public static void main(String[] args) throws IOException {
		...
		/*
		시나리오] 1부터 1000까지의 정수중 4의배수이거나 7의배수인 수의 합을 구하여 출력하는 프로그램을 작성.
		단, while문을 사용
		 */
		
		int total = 0; //합을 구하기 위한 변수
		int j = 1; //반복을 위한 변수
		do { //조건 검사 없이 무조건 진입 후 실행
			if(j%4==0 || j%7==0) { //4 or 7의 배수일 때
				System.out.println("j="+ j);
				total += j; //j를 누적하여 더함
			}
			j++; 
		}while(j<=1000); //세미콜론으로 마무리
		...
		단, 사용자가 X, x(대소문자 구별없음)를 입력하면 프로그램이 종료되어야 하며 do~while문으로 구현 해야한다.
		 */
		...
		do {
			...
			산술식만 사용할 수 있으므로 점수의 구간을 구하기 위해 총점을 30으로 나눠준다.
			 */
		...
		/*
		X를 입력한 경우
			: (false or true) -> (true) //OR 둘중 하나만 t면 t
								!(true) -> false 반환
			do~while문을 탈출한다.
